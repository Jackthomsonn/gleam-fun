...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, AJS} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, AJS} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, AJO} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, AJO} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, AJK} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, AJK} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec nullable(gleam@option:option(AJY), fun((AJY) -> json())) -> json().
...
-spec array(list(AKC), fun((AKC) -> json())) -> json().
